    ...
    try:
        ...
        pg_cursor.execute("""
            SELECT EXISTS (
                ...
                WHERE table_name = 'ciscocdr_calldetails_mini'
            )
        """)
        ...
        if table_exists:
            ...
            pg_cursor.execute("SELECT MAX(\"dateTimeOrigination\") FROM ciscocdr_calldetails_mini")
...
def create_postgresql_table_if_not_exists(pg_cursor, column_names, logger):
    """
    Создает таблицу ciscocdr_calldetails_mini в PostgreSQL, если она не существует.
    """
    create_table_query = f"""
        CREATE TABLE IF NOT EXISTS ciscocdr_calldetails_mini (
            {', '.join([f'"{col}" TEXT' for col in column_names])}
        )
    """
    pg_cursor.execute(create_table_query)
    logger.info(f"Таблица ciscocdr_calldetails_mini создана/проверена с {len(column_names)} колонками")


def main():
    """
    Основная функция для переноса данных из MySQL в PostgreSQL.
    Получает максимальную дату из PostgreSQL таблицы ciscocdr_calldetails_mini и собирает
    ...
    try:
        ...
        if data:
            ...
            insert_query = f"""
                INSERT INTO ciscocdr_calldetails_mini ({', '.join([f'"{col}"' for col in column_names])})
